...
namespace OA.MVC.Common.Services
{
    public class LayoutService
    {
        ...
        /// <returns></returns>
        public HtmlString RenderSideBar(HttpContext context, HtmlType type)
        {
            var titlelist = _menu.GetTitle();
            string sidebar = "";
            string bread = "";
            string[] arr = context.Request.Path.ToString().Split(new Char[] { '/' });
            if (arr.Count() <= 2)
            {
            ...
            if (titlelist != null)
            {
                foreach (var title in titlelist)
                {
                    ...
                    var menulist = _menu.GetSubMenu(title.Id);
                    foreach (var item in menulist)
                    {
                        if (item.Action == arr[2] && item.Controller == arr[1])
                        {
